...
class AuthController extends BaseController
{
    public function register($req, $response)
    {
        ...
        if ($row[0] > 0) {
            return $response->withStatus(400)->withJson(["error" => true, "message" => "User $e already exists."]);
        }

        if ($rowc[0] > 0) {
            $stmt = $this->mysqli->prepare("INSERT INTO users (name, email, password, type) VALUES (?, ?,  ?, 'user');");
        } else {
            $stmt = $this->mysqli->prepare("INSERT INTO users (name, email, password, type) VALUES (?, ?,  ?, 'admin');");
        }
        ...
        if (isset($_ENV['SECRET_KEY'])) {
            ...
            $key = 'Project Amihan --- this is a secret please change this.';
        }
    ...
    public function authenticate($req, $response)
    {
        ...
        $row = $stmt->get_result()->fetch_array();
        if (!isset($row[0])) {
            return $response->withStatus(401)->withJson(["error" => true, "message" => "User not found"]);
        }

        if (!password_verify($p, $row['password'])) {
            return $response->withStatus(401)->withJson(["error" => true, "message" => "Incorrect Password"]);
        }
